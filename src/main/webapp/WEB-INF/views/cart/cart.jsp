				...
				$.ajax({
					...
					success:function(Result){
						...
						for(var i in jdata.jlist){
							...
							output += '<td colspan="4">';
							output += '<div class="price_content"><span class="p1_price">'+ jdata.jlist[i].pprice +'</span>원</div>';
							output += '</td></tr>';
					...
					<tr class="cart_product">
						...
						<td colspan="4">
							<div class="cart_text"><a href="http://localhost:9000/sistproject3/product.do?pid=${vo.pid }">
								<label>${vo.p_name }</label></a>
							</div>
						</td>
					</tr>
					<tr class="cart_explain">
						<td width=53%>
							<div class="cart_option">${vo.p_opt }</div>
						</td>
						<td width=12%>
							<div class="cart_num">
								<button type="button" class="minus" name="minus" id="p1">-</button>
								<input type="text" class="price" value="${vo.p_qty }" id="p1_amt">
								<button type="button" class="plus" name="plus" id="p1">+</button>
							</div>
						</td>
						<td width=11%>
							<div class="cart_price"><span class="p1_price">${vo.p_price }</span>원</div>
						</td>
						<td>
